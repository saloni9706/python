...
# This program is not repeating quotes in output ; PART E is already solved in Part-D.
...
if query.find("AND")!= -1:
    ...
    print("Enter either AND or OR keyword")
    exit()
lower_words=[]
for word in words:
    lower_word=word.lower()
    lower_words.append(lower_word)
fisrt_query=lower_words[0]
second_query=lower_words[1]
for quote in data_list:

    lower_quote=quote.lower()

    found_at_first=lower_quote.find(fisrt_query)
    found_at_second=lower_quote.find(second_query)
    if keyword_type=="and":
        if (found_at_first>=0 and found_at_second>=0):
            print(quote,"\n")

    elif keyword_type=="or":
        if(found_at_first>=0 or found_at_second>=0) :
            print(quote,"\n")
    found_at_first=-1
    found_at_second=-1
