# Query term seperated with white spaced searching with CASE SENSITIVE.
...
for quote in data_list:
    # Since the indices won't change by just converting the entire string to upper or lower cases,therefore 
    # convert the entire string to lower case
    lower_quote=quote.lower()

    # now convert to query to lower case ; since find() function is case sensitive therefore, we need to convert the query to lowrcase as well.
    lower_query=query.lower()

    # Find the position of the lowercase query in lowercase string or quote
    # found_at=lower_quote.find(lower_query)
    all_words=False
    found_at_index=[]
    for lower_word in lower_words:
        found_at=lower_quote.find(lower_word)
        if(found_at>=0):
            all_words=True
            found_at_index.append(found_at)
        else:
            all_words=False
            break
    if all_words==True:
        print("Found:","..."+quote[found_at_index[0]:found_at_index[len(found_at_index)-1]+50],"...")
